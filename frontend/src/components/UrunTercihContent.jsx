...
function UrunTercihContent() {
    ...
    useEffect(() => {
        ...
        if (loggedInUser) {
            ...
            // Eğer her iki değer de yoksa, misafir için bir sessionId oluşturabiliriz
    ...
    // "Tercih Edilenler" için filtreleme
    const preferredProducts = products.filter(product => product.tags && product.tags.includes('Tercih Edilen'));

    const filteredProducts = preferredProducts.filter(product => {
        const matchesName = product.name.toLowerCase().includes(filter.name.toLowerCase());
        const matchesCategory = filter.category ?
            (product.category.includes(filter.category)) : true;
        const matchesColor = filter.color ?
            (typeof product.color === 'string' && product.color.toLowerCase() === filter.color.toLowerCase()) : true;
        const matchesSize = filter.size ?
            (product.size && product.size.includes(filter.size)) : true;
    ...
    const categories = [...new Set(products.map(product => product.category[0]))];
        ...
        <div className="container mx-auto px-4 py-8">
            ...
            <div className='mt-16 flex flex-col border-b-2 justify-between items-start'>
                ...
                <div className='flex flex-row items-center montserrat gap-2 mb-4'>
                    ...
                    <div className='py-4'>
                        <h1 className='text-lg font-semibold text-gray-800'>Tercih Edilenler</h1>
                    </div>
                </div>
            ...
            <div
                ...
                ref={filterRef} // Filtre panelini referans alıyoruz
            >
                <div className="p-4 relative">
                    ...
                    <div className='flex flex-col mt-12 items-start text-sm justify-center'>
                        ...
                        <select
                            ...
                            className="border rounded text-sm p-2 mt-4 w-full"
                        >
                        ...
                        <div className="flex mt-4">
                            <input
                                ...
                                className="border text-sm rounded p-2 mr-2 w-full"
                            />
                            <input
                                type="number"
                                ...
                                className="border text-sm rounded p-2 w-full"
                            />
                        </div>
                                ...
                                >
                                    {product.imageUrls && product.imageUrls.length > 0 ? (
                                        <div className="relative w-full h-full">
                                             <div className='absolute z-10 bg-yellow-500 p-2 raleway text-xs text-white  top-2 left-2'>
                                <h1>Tercih Edilen</h1>
                                </div>
                                            <img
...
export default UrunTercihContent;